...
import com.rs2.gameevents.DeathEvent;
...
public class PlayerManager {
	...
	static { 
		...
		Runtime.getRuntime().addShutdownHook(new Thread(){
			public void run() {
				...
				synchronized(this) {
					for (Player p : getSingleton().getPlayers()) {
						if (p == null)
							continue;
						if (((Client)p).getTradeHandler().stage > 0) {
							System.out.println("Did not save "+p.getUsername()+" as they are in a trade.");
							continue;
						}
						saved++;
						getSingleton().saveGame((Client)p, true);
	...
	public void process() {
		...
		for (int i = 0; i < Constants.MAX_PLAYERS; i++) {
			...
			FollowEngine.loop((Client) players[i]);
			...
			if (players[i].disconnected) {
				if (client.getTradeHandler().getStage() > 0) {
					client.getTradeHandler().cancelCurrentTrade();
				}
			}
		...
		for (int i = 1; i < NPCManager.MAXIMUM_NPCS; i++) {
			if (GameEngine.getNPCManager().getNPC(i) != null) {
				...
				if (plr.rebuildNPCList == false
					...
					plr.addNewNPC(GameEngine.getNPCManager().getNPC(i), str,
							updateBlock);
				}
			}
		}
	...
	public boolean saveGame(Player p, boolean loggout) {
		try {
			...
			s.executeUpdate("update characters set absX = '" + playerDetails.absX + "', absY = '" + playerDetails.absY + "', heightLevel = '" + playerDetails.heightLevel + "', hitpoints = '" + playerDetails.hitpoints + "', specialAmount = '" + playerDetails.specialAmount + "', poisonDelay = '" + playerDetails.poisonDelay + "', poisonDamage = '" + playerDetails.poisonDamage + "', skullTimer = '" + playerDetails.skullTimer + "', pkPoints = '" + playerDetails.pkPoints + "', energy = '" + playerDetails.energy + "', ancients = '" + (playerDetails.ancients ? 1:0) + "', playerIsMember = '" + (playerDetails.playerIsMember ? 1:0) + "', dharokDamage = '" + playerDetails.dharokDamage + "', spellBook = '" + playerDetails.spellBook + "', appearanceSet = '" + (playerDetails.appearanceSet ? 1:0) + "', ip = '" + playerDetails.ConnectedFrom + "', splitChat = '" + (playerDetails.SplitChat ? 1 : 0) + "', running = '" + playerDetails.running + "', autoRetaliate = '" + (playerDetails.autoRetaliate ? 1 : 0) + "', combatMode = '"+ (playerDetails.combatMode) +"', lastmeleemode = '"+ (playerDetails.lastMeleeMode) +"', lastrangemode = '"+ (playerDetails.lastRangeMode) +"', starter = '"+ playerDetails.starter +"', online = '"+ online +"' WHERE ID = '" + playerDetails.ID + "'");
			String[] delete = {"banks", "friends", "ignores", "inventorys", "equipments", "looks", "skills", "lastkilled"};
			
			for (int i = 0; i < delete.length; i++)
				s.executeUpdate("DELETE FROM " + delete[i] + " WHERE ID = '" + playerDetails.ID + "'");
			
			for (int i =0; i < playerDetails.bankItems.length; i++) {
				long id = playerDetails.bankItems[i];
				if(id > 0 && id != -1)
					s.executeUpdate("INSERT INTO banks (ID,item,amount,slot) VALUES ('" + playerDetails.ID + "', '" + id + "', '" + playerDetails.bankItemsN[i] + "', '" + i + "')");
			}
			
			for(int i =0; i < playerDetails.playerItem.length; i++){
				long id = playerDetails.playerItem[i];
				if(id > 0 && id != -1)
					s.executeUpdate("INSERT INTO inventorys (ID,item,amount,slot) VALUES ('" + playerDetails.ID + "', '" + id + "', '" + playerDetails.playerItemN[i] + "', '" + i + "')");
			}
			
			for(int i =0; i < playerDetails.playerEquipment.length; i++){
				long id = playerDetails.playerEquipment[i];
				if(id > 0 && id != -1)
				s.executeUpdate("INSERT INTO equipments (ID,item,amount,slot) VALUES ('" + playerDetails.ID + "', '" + id + "', '" + playerDetails.playerEquipmentN[i] + "', '" + i + "')");
			}
			
			for(int i =0; i < playerDetails.playerLevel.length; i++){
				s.executeUpdate("INSERT INTO skills (ID,skill,xp,slot) VALUES ('" + playerDetails.ID + "', '" + playerDetails.playerLevel[i] + "', '" + playerDetails.playerXP[i] + "', '" + i + "')");
			}
			
			for(int i =0; i < playerDetails.playerLook.length; i++){
				s.executeUpdate("INSERT INTO looks (ID,look,slot) VALUES ('" + playerDetails.ID + "', '" + playerDetails.playerLook[i] + "', '" + i + "')");
			}
			
			for(int i =0; i < playerDetails.lastKilled.length; i++){
				if (playerDetails.lastKilled[i] != null)
					s.executeUpdate("INSERT INTO lastkilled (ID,username,killed,slot) VALUES ('" + playerDetails.ID + "', '" + playerDetails.username + "', '" + playerDetails.lastKilled[i] + "', '" + i + "')");	
			}
			
			for(int i =0; i < playerDetails.friends.length; i++){
				long friend = playerDetails.friends[i];
				if(friend > 0)
					s.executeUpdate("INSERT INTO friends (ID,friend,slot) VALUES ('" + playerDetails.ID + "', '" + friend + "', '" + i + "')");
			}
			
			for(int i =0; i < playerDetails.ignores.length; i++){
				long ignore = playerDetails.ignores[i];
				if(ignore > 0)
					s.executeUpdate("INSERT INTO ignores (ID,ignored,slot) VALUES ('" + playerDetails.ID + "', '" + ignore + "', '" + i + "')");
			}
