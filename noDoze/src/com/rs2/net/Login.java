...
public class Login {
	...
	private Stream outStream = new Stream(new byte[Constants.BUFFER_SIZE]);
	private Stream inStream = new Stream(new byte[Constants.BUFFER_SIZE]);
	...
	public void process() throws Exception, IOException {
		...
		if (state == State.LOGIN_READ1) {
			if (fillInStream(2)) {
				...
				Misc.print_debug("LoginPacket size: " + loginPacketSize
		...
		if (state == State.LOGIN_READ2) {
			if (fillInStream(loginPacketSize)) {
				...
				Misc.print_debug("Client type: "
						+ ((lowMemoryVersion == 1) ? "low" : "high")
						+ " memory version");
				for (int i = 0; i < 9; i++) {
					Misc.print_debug("dataFileVersion[" + i + "]: 0x"
				...
				username = Misc.formatUsername(inStream.readString());
					...
					try {
						...
						if (rs.next()) {
							if (!password.equals(rs.getString("password"))) {
								...
								if (location[0] == -1 || location[1] == -1) { //blackscreen fix. - killamess
									location[0] = PlayerConstants.SPAWN_X;
									location[1] = PlayerConstants.SPAWN_Y;
									location[2] = 0;
								}
							...
							s.executeQuery ("SELECT * FROM inventorys WHERE ID = '" + c.ID + "'");
							rs = s.getResultSet();
							while(rs.next()) {
								int i = rs.getInt("slot");
							...
							s.executeQuery ("SELECT * FROM equipments WHERE ID = '" + c.ID + "'");
							rs = s.getResultSet();
							while(rs.next()) {
								int i = rs.getInt("slot");
							...
							s.executeQuery ("SELECT * FROM banks WHERE ID = '" + c.ID + "'");
							rs = s.getResultSet();
							while(rs.next()) {
								int i = rs.getInt("slot");
							...
							s.executeQuery ("SELECT * FROM skills WHERE ID = '" + c.ID + "'");
							rs = s.getResultSet();
							while(rs.next()) {
								int i = rs.getInt("slot");
							...
							s.executeQuery ("SELECT * FROM looks WHERE ID = '" + c.ID + "'");
							rs = s.getResultSet();
							while(rs.next()) {
								int i = rs.getInt("slot");
							...
							s.executeQuery ("SELECT * FROM friends WHERE ID = '" + c.ID + "'");
							rs = s.getResultSet();
							...
							s.executeQuery ("SELECT * FROM ignores WHERE ID = '" + c.ID + "'");
							rs = s.getResultSet();
			...
			if (c != null) {
					...
					if (BanProcessor.checkMuted(c.getUsername()) || BanProcessor.checkIpMuted(c.getUsername())) {
						c.setMuted(true);
					}
					System.out.println("" + c.getUsername() + " logged in [idx="
							+ c.getUserID() + ", online="
				...
				directFlushOutStream();
				if (returnCode == 2) {
					GameEngine.getIOThread().switchIoClientToClient(this, c);
				} else {
		...
		try {
			try {
				...
				GameEngine.getIOThread().destroySocket(
						GameEngine.getIOThread().socketFor(this),
						connectedFrom, true);
			}
		} catch (Exception e) {
		}
