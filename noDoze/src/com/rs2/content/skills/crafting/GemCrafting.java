...
public class GemCrafting implements Task {
	...
	public enum gemData {
		...
		diamond(1601, 43, (int) 107.5, 43, 85, 1643, 56, 90, 1662, 70, 100, 1681, 1700),
		dragon_stone(1615, 55, (int) 137.5, 55, 100, 1645, 72, 105, 1664, 80, 150, 1683, 1702),
		onyx(6573, 67, (int) 167.5, 67, 115, 6575, 82, 120, 6577, 90, 165, 6579, 6581);

		public int[] mapLocation = new int[13];

		gemData(int a, int b, int c, int d, int e, int f, int g, int h, int i, int j, int k, int l, int m) {
			mapLocation[0] = a;
			mapLocation[1] = b;
			mapLocation[2] = c;
			mapLocation[3] = d;
	...
	public void loop(Action currentAction) {
		...
		if (getData(client.gem) == null || !client.getActionAssistant().isItemInBag(getData(client.gem).mapLocation[0]) || !client.getActionAssistant().isItemInBag(2357)) {
			stop(currentAction);
			return;
		}
		...
		case 0: 
			...
			if (client.playerLevel[PlayerConstants.CRAFTING] < getData(client.gem).mapLocation[3]) {
    			client.getActionSender().sendMessage("You need a Crafting level of "+ getData(client.gem).mapLocation[3] +" to craft this.");
    			stop(currentAction);
    			return;
			}
			client.getActionAssistant().deleteItem(getData(client.gem).mapLocation[0], 1);
			
			if (getData(client.gem).mapLocation[0] != 2357) 
				client.getActionAssistant().deleteItem(2357, 1);
			
			client.getActionSender().sendInventoryItem(getData(client.gem).mapLocation[5], 1);
			client.getActionSender().sendMessage("You craft an ring.");
			client.getActionAssistant().addSkillXP(getData(client.gem).mapLocation[4] * PlayerConstants.SKILL_EXPERIENCE_MULTIPLIER, PlayerConstants.CRAFTING);
			break;


		case 1:
			if (client.playerLevel[PlayerConstants.CRAFTING] < getData(client.gem).mapLocation[6]) {
    			client.getActionSender().sendMessage("You need a Crafting level of "+ getData(client.gem).mapLocation[6] +" to craft this.");
    			stop(currentAction);
    			return;
			}
			client.getActionAssistant().deleteItem(getData(client.gem).mapLocation[0], 1);
			...
			client.getActionSender().sendInventoryItem(getData(client.gem).mapLocation[8], 1);
	 			client.getActionSender().sendMessage("You craft an necklace.");
			client.getActionAssistant().addSkillXP(getData(client.gem).mapLocation[7] * PlayerConstants.SKILL_EXPERIENCE_MULTIPLIER, PlayerConstants.CRAFTING);
			break;


		case 2: 
			if (client.playerLevel[PlayerConstants.CRAFTING] < getData(client.gem).mapLocation[9]) {
    			client.getActionSender().sendMessage("You need a Crafting level of "+ getData(client.gem).mapLocation[9] +" to craft this.");
    			stop(currentAction);
    			return;
			}
			client.getActionAssistant().deleteItem(getData(client.gem).mapLocation[0], 1);
			...
			if (client.getActionAssistant().isItemInBag(1759)) {
				...
				client.getActionSender().sendInventoryItem(getData(client.gem).mapLocation[12], 1);
				client.getActionAssistant().addSkillXP(30 * PlayerConstants.SKILL_EXPERIENCE_MULTIPLIER, PlayerConstants.CRAFTING);
    			client.getActionSender().sendMessage("You craft an amulet and attach a string to it.");
			} else {
				client.getActionSender().sendInventoryItem(getData(client.gem).mapLocation[11], 1);
    			client.getActionSender().sendMessage("You craft an amulet.");
			}
			client.getActionAssistant().addSkillXP(getData(client.gem).mapLocation[10] * PlayerConstants.SKILL_EXPERIENCE_MULTIPLIER, PlayerConstants.CRAFTING);
			break;
		...
		if (client.craftingAmount > 0 && client.getActionAssistant().isItemInBag(getData(client.gem).mapLocation[0]) && client.getActionAssistant().isItemInBag(2357))
	...
	public static void startCrafter(Client client, int gem, int actionAmount, int type) {
		...
		case 2:
			...
			if (client.playerLevel[PlayerConstants.CRAFTING] < getData(gem).mapLocation[count * 3]) {
				client.getActionSender().sendMessage("You need a crafting level of "+getData(gem).mapLocation[count * 3]+" to craft this item.");
				return;
			}
			break;	
		}
		if (!client.getActionAssistant().isItemInBag(getData(gem).mapLocation[0x0]) || !client.getActionAssistant().isItemInBag(2357)) {
			client.getActionSender().sendMessage("You do not have the required items.");
			return;
		}
		client.getActionAssistant().startAnimation(899, 0);
		client.setBusy(true);
	...
	public static void string(Client client, int jemSlot) {
		if (!client.getActionAssistant().isItemInBag(1759) || !client.getActionAssistant().isItemInBag(mendItems[jemSlot][0])) {
			return;
        }
		client.getActionSender().sendWindowsRemoval();
		...
		client.getActionAssistant().addSkillXP(30 * PlayerConstants.SKILL_EXPERIENCE_MULTIPLIER, PlayerConstants.CRAFTING);
       	client.getActionSender().sendMessage("You attach an string to the amulet.");
	}
	
	public static void craftGem(Client client, int toCut) {
		
		if (!client.getActionAssistant().isItemInBag(1755) || !client.getActionAssistant().isItemInBag(toCut)) 
			return;
        
		for (int i = 0; i < craftGem.length; i++) {
			if (toCut == craftGem[i][0]) {
				if (client.playerLevel[PlayerConstants.CRAFTING] < craftGem[i][2]) {
					client.getActionSender().sendMessage("You need a crafting level of "+ craftGem[i][2] +" to cut this gem.");
					return;
				}
				client.getActionSender().sendWindowsRemoval();
				...
				client.getActionAssistant().addSkillXP(craftGem[i][3] * PlayerConstants.SKILL_EXPERIENCE_MULTIPLIER, PlayerConstants.CRAFTING);
				break;
			}
		}
	}
	...
	public static final int[][] SILVER = {
	 	{1716, 16}, {1724, 17}, {2961, 18}, {5525, 20},
	};
	
	/**
	 * Amulets used with strings
	 * unattachedId, attachedId
	 */
	public static int[][] mendItems = {
		{1673, 1692 },  { 1675, 1694 }, { 1677, 1696 }, { 1679, 1698 },
		{ 1681, 1700 }, { 1683, 1702 }, { 6579, 6581 }
	};
	
	/**
	 * What appears as our rings, necklaces and amulets in
	...
	public static final int[][] ITEMS = {
		//itemid, baseid, type.
		{1635, 2357, RING}, {1654, 2357, NECKLACE}, {1673, 2357, AMULET},
		{1637, 1607, RING}, {1656, 1607, NECKLACE}, {1675, 1607, AMULET},
		{1639, 1605, RING}, {1658, 1605, NECKLACE}, {1677, 1605, AMULET},
		{1641, 1603, RING}, {1660, 1603, NECKLACE}, {1679, 1603, AMULET},
		{1643, 1601, RING}, {1662, 1601, NECKLACE}, {1681, 1601, AMULET},
		{1645, 1615, RING}, {1664, 1615, NECKLACE}, {1683, 1615, AMULET},
		{6575, 6573, RING}, {6577, 6573, NECKLACE}, {6579, 6573, AMULET}
	};
	
	/**
	 * Used for cutting uncut gems
	 * uncutid, cutid, level, exp, animation
	 */
	public static final int[][] craftGem = {
		{1623, 1607 , 20, 50, 888},
		{1621, 1605 , 27, 67, 889},
		{1619, 1603 , 34, 85, 887},
		{1617, 1601 , 43, (int) 107.5, 886},
		{1631, 1615 , 55, (int) 137.5, 885},
		{6571, 6573 , 67, 168, 2717}
