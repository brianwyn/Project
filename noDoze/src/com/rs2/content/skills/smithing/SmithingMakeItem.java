package com.rs2.content.skills.smithing;
...
public class SmithingMakeItem implements Task {
	...
	public boolean doaction(final Client c, final int toadd, final int barID, final int barsRemoved, final int timestomake, final int NOTUSED, final int NOTUSED2, final int xp) {
		if(c == null){
			return false;
		}
	...
	public void loop(Action currentAction) {
		...
		if (!client.getActionAssistant().playerHasItem(client.barID,client.barsRemoved)){
			client.getActionSender().sendMessage("You don't have enough bars to make this item!");
			stop(currentAction);
			return;
		}
		...
		if (client.amountToMakeBars > 1 && client.getActionAssistant().playerHasItem(client.barID, client.barsRemoved * 2)){
			client.getActionSender().sendMessage("You make some " + GameEngine.getItemManager().getItemDefinition((client.itemToAdd)).getName() +"s");
		} else {
			client.getActionSender().sendMessage("You make a " + GameEngine.getItemManager().getItemDefinition((client.itemToAdd)).getName());
		}
		if (client.getActionAssistant().playerHasItem(client.barID, client.barsRemoved)) {
			if (client.barsRemoved == 1) {
				client.getActionAssistant().deleteItem(client.barID, client.barsRemoved);
		...
		if (GameEngine.getItemManager().getItemDefinition((client.itemToAdd)).getName().contains("dart")){
			client.getActionSender().sendInventoryItem(client.itemToAdd, 10);
		} else if (GameEngine.getItemManager().getItemDefinition((client.itemToAdd)).getName().contains("nail")){
			client.getActionSender().sendInventoryItem(client.itemToAdd, 15);
		} else if (GameEngine.getItemManager().getItemDefinition((client.itemToAdd)).getName().contains("arrow")){
			client.getActionSender().sendInventoryItem(client.itemToAdd, 15);
		} else if (GameEngine.getItemManager().getItemDefinition((client.itemToAdd)).getName().contains("knife")){
			client.getActionSender().sendInventoryItem(client.itemToAdd, 5);
		} else if (GameEngine.getItemManager().getItemDefinition((client.itemToAdd)).getName().contains("Cannonball")){
	...
	public void readInput(int level, String type, Client c, int amounttomake,int NOTUSED, int NOTUSED2) {

	String Name = GameEngine.getItemManager().getItemDefinition(Integer.parseInt(type)).getName();
	...
	private void CheckIron(Client c, int level, int amounttomake, String type) {
		...
		doaction(c, c.smithitem, c.smithremove, c.smithremoveamount, c.smithmaketimes, -1, -1, c.smithxp);
		
	}

	private void CheckSteelCannonBalls(Client c, int level, int amounttomake, String type) {
		c.smithremove = 2353;
