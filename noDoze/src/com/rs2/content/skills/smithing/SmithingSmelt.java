...
public class SmithingSmelt implements Task {
	...
	private String itemidforstring(int itemid, Client c) {
		if (itemid == 438 || itemid == 436)
			...
			return "RUNE";
		else if (itemid == 443 && c.getActionAssistant().playerHasItem(453, 2) && c.getActionAssistant().playerHasItem(440, 1))
			return "STEEL";
		else if (itemid == 668)
			return "BLUR";
		else if (itemid == 2892)
	...
	public boolean doaction(final Client client, final int toadd, final int toremove, final int toremove2, final int timestosmelt, final int removeam, final int removeam2, final int xp) {
	...
	public void readInput(int level, String type, Client c, int objectx, int objecty, int objectid) {
		...
		synchronized (this) {
			if (type.equals("BRONZE") && level >= 1) {
				if (c.getActionAssistant().playerHasItem(438, 1) && c.getActionAssistant().playerHasItem(436, 1)) {
					c.xp = 6;
					c.item = 2349;
					c.remove = 438;
					c.remove2 = 436;
					...
					c.getActionSender().sendWindowsRemoval();
					c.getActionSender().sendMessage("You don't have enough tin to smelt this bar!");
					return;
				} else {
					c.getActionSender().sendWindowsRemoval();
					c.getActionSender().sendMessage("You don't have enough copper to smelt this bar!");
					return;
				}
				...
				if (c.getActionAssistant().playerHasItem(453, 4)) {
					...
					c.getActionSender().sendWindowsRemoval();
					c.getActionSender().sendMessage("You don't have enough coal to smelt this bar!");
					return;
				}
				...
				c.getActionSender().sendMessage("You don't have a high enough smithing level to smelt this bar!");
				c.getActionSender().sendWindowsRemoval();
				return;
			}
			c.getActionSender().sendWindowsRemoval();
			doaction(c, c.item, c.remove, c.remove2, objectx, c.removeam, c.removeam2, c.xp);			
		}		
	}
	...
	public void loop(Action currentAction) {
		...
		if(!client.getActionAssistant().playerHasItem(client.toremove,1) || !client.getActionAssistant().playerHasItem(client.barsRemoved,client.removeam2) && client.barsRemoved > 0){
		...
		} else if (client.toremove == 440 && client.barsRemoved != 453 && Misc.random(2) == 1) {
			client.getActionAssistant().deleteItem(440, client.getActionAssistant().getItemSlot(440), 1);
			client.getActionSender().sendMessage("You fail to smelt the iron ore into a bar.");
			Animation.createAnimation(client, 899);				
		...
		if (!client.hasfailed2123) {
			client.getActionSender().sendMessage("You smelt the ore into a bar.");
			client.getActionAssistant().addSkillXP(client.xp, 13);
			client.getActionAssistant().replaceItem(client.toremove, client.item);
		}
		...
		if (client.amountToMakeBars > 0) {
			...
			if (client.barsRemoved > 0) {
				if (client.removeam2 > 0) {
					if (!client.getActionAssistant().playerHasItem(client.barsRemoved, client.removeam2)) {
						failFlag = true;
					}
				}
			}
