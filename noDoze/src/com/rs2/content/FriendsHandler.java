...
public class FriendsHandler {
	...
	public void initialize() {
		...
		refreshLists();
		long l = Misc.playerNameToInt64(client.getUsername());
	...
	public int getWorld(long who) {
		Player p = PlayerManager.getSingleton().getPlayerByNameLong(who);
		int world = 0;
		if (p != null) {
			world = 1;
		}
	...
	public void sendMessage(long to, byte[] chatText, int chatTextSize) {
		Player p = PlayerManager.getSingleton().getPlayerByNameLong(to);
		Client c = (Client) p;
		if (c == null) {
			client.getActionSender().sendMessage(
					Misc.longToPlayerName(to) + " is currently unavailable.");
			return;
		} else {
			c.getFriendsHandler().sendPM(
					Misc.playerNameToInt64(client.getUsername()),
