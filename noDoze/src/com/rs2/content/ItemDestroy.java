...
public class ItemDestroy {
	...
	public static void option(Client client, int itemId) {
		String itemName = GameEngine.getItemManager().getItemDefinition(itemId).getName();
		String[][] info = {
		...
		for (int i = 0; i < info.length; i++)
			client.getActionSender().sendQuest(info[i][0], Integer.parseInt(info[i][1]));
	...
	public static void destroyItem(Client client) {
		String itemName = GameEngine.getItemManager().getItemDefinition(client.destroyItem).getName();
		client.getActionAssistant().deleteItem(client.destroyItem, 1);
		client.getActionSender().sendMessage("Your " + itemName.toLowerCase() + " vanishes as you drop it on the ground.");
