...
public class OreSmelter implements Task {
	...
	public void loop(Action currentAction) {
		if (currentAction.getClient().oreId < 1 || currentAction.getClient().smeltAmount < 1) {
			stop(currentAction);
			return;
		}
		...
		if (currentAction.getClient().getActionAssistant().playerHasItem(currentAction.getClient().oreId, 1)) {
			for (int i = 0; i < oreConverter.length; i++) {
				if (oreConverter[i][0] == currentAction.getClient().oreId) {
					if (currentAction.getClient().playerLevel[PlayerConstants.CRAFTING] < oreConverter[i][2]) {
						currentAction.getClient().getActionSender().sendMessage("You need a smithing level of "+ oreConverter[i][2]+" to smith "+ GameEngine.getItemManager().getItemDefinition(oreConverter[i][0]).getName().toLowerCase()+".");
						stop(currentAction);
						return;	
					}
					...
					currentAction.getClient().getActionAssistant().deleteItem(currentAction.getClient().oreId, 1);
					currentAction.getClient().getActionSender().sendInventoryItem(oreConverter[i][1], 1);
					currentAction.getClient().getActionAssistant().addSkillXP(oreConverter[i][3] * PlayerConstants.SKILL_EXPERIENCE_MULTIPLIER, PlayerConstants.SMITHING);
					break;
				}
			}
			
		} else {
			currentAction.getClient().getActionSender().sendMessage("You have ran out of ore.");
			stop(currentAction);
			return;
		}
	...
	public static final int[][] oreConverter = {
		{ 444, 2357, 40, (int) 22.5}, //gold
	};
