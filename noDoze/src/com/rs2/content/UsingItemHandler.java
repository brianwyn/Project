...
public class UsingItemHandler {
	...
	public static final int[] FOODS = { 1971, 319, 315, 2140, 2142, 1901, 2309, 347,
			355, 333, 351, 329, 361, 379, 365, 373, 7946, 385, 397, 391 };
	...
	public static void useItem(Client client, int itemID, int itemSlot) {
		...
		if (client.isDead() || action == Action.NOTHING || client.isBusy() || client.getBusyTimer() > 0 || client.getStunnedTimer() > 0) 
			return;
		...
		if (action == Action.EAT_FOOD) {
			...
			case 1971: //kebab
				...
				int randomNumber = Misc.random(2);
				
				switch(randomNumber) {
				
				case 0:
					if (Misc.random(15) == 7) {
						client.getActionSender().sendMessage("That was an excellent Kabab!");
						healAmount = 7 + Misc.random(5);
						break;
					} else {
						client.getActionSender().sendMessage("You eat the kebab.");
						healAmount = 2;
						break;
					}
				...
				case 1:
					if (Misc.random(10) == 6) {
						client.getActionSender().sendMessage("That was one yummy Kabab!");
				...
				case 2:
					if (Misc.random(1) == 1) {
						client.getActionSender().sendMessage("You eat the kebab, it makes your feel sick.");
						healAmount = 0;
						Damage.addNewHit(client, client, CombatType.POISON, 2 + Misc.random(1) == 0 ? 2 : 4, 4);
						break;
					} else {
						client.getActionSender().sendMessage("You eat the kebab.");
						healAmount = 2;
			...
			((Entity)client).setCombatDelay(((Entity)client).getCombatDelay() + 3);
			CombatEngine.resetAttack(client, false);
			if (itemID != 1971) 
			client.getActionSender().sendMessage(
					"You eat the "
							+ GameEngine.getItemManager().getItemDefinition(
									itemID).getName().toLowerCase() + ".");
			...
			client.getActionSender().sendMessage(
					"You bury the "
							+ GameEngine.getItemManager().getItemDefinition(
									itemID).getName().toLowerCase() + ".");
			client.getActionAssistant().addSkillXP(buryXP * PlayerConstants.COMBAT_EXPERIENCE_MULTIPLIER, PlayerConstants.PRAYER);
		} else if (action == Action.DRINK_POTION) {
			switch (itemID) {
			...
			case 1978:
				...
				client.getActionSender().sendMessage("Ahhhhhh... nothing like a nice cup of tea.");
				break;
			...
			CombatEngine.resetAttack(client, false);
			client.getActionAssistant().increaseStat(skillToIncrease,percentageIncrease);
			client.getActionAssistant().deleteItem(itemID, itemSlot, 1);
			if (newItem > 0)
				client.getActionSender().sendInventoryItem(newItem, 1, itemSlot);
			client.getActionAssistant().startAnimation(829, 0);
			if (newItem != 1980)
				client.getActionSender().sendMessage("You drink some of the " + GameEngine.getItemManager().getItemDefinition(itemID).getName().toLowerCase() + ".");
		}
	}
}
