...
public class TradeHandler {
	...
	public void requestTrade(Client otherClient) {
		...
		if (client.getPrivileges() >= 1 && otherClient.getPrivileges()  == 0 && !(client.getPrivileges() >= 3)) {
			client.getActionSender().sendMessage("You are restricted from trading non-ranked players.");
			return;
		}
		if (client.getPrivileges() == 0 && otherClient.getPrivileges()  >= 1 && !(client.getPrivileges() >= 3)) {
			client.getActionSender().sendMessage("You are restricted from trading ranked players.");
			return;
		}
		...
		otherClient.getActionSender().sendMessage(client.getUsername() + ":tradereq:");
	...
	public void answerTrade(Client otherClient) {
		...
		if (currentTrade == null) {
			...
			if (currentTrade.isOpen()) {
				...
				client.getActionSender().sendQuest("TradeHandler With: "+ otherClient.getUsername(), 3417);
				client.getActionSender().sendQuest("", 3431);
				...
				otherClient.getActionSender().sendQuest("TradeHandler With: " + client.getUsername(), 3417);
				otherClient.getActionSender().sendQuest("", 3431);
				accepted = false;
				GameEngine.getPlayerManager().saveGame((Client) client, false);
				GameEngine.getPlayerManager().saveGame((Client) otherClient, false);
	...
	public void decline() {
		Client other = getOther();
		if (other != null) {
			other.getActionSender().sendMessage(client.getUsername() + " has declined the trade.");
	...
	public void cancelCurrentTrade() {
		if (currentTrade.isOpen()) {
			...
			currentTrade.getEstablisher().getActionSender().sendWindowsRemoval();
	...
	public void acceptStage1() {
		...
		if (currentTrade != null && currentTrade.isOpen()) {
			...
			if (itemsTraded() > other.getActionAssistant().freeSlots()) {
				client.getActionSender().sendMessage("The other player doesn't have enough space left in their inventory.");
				return;
			}
			if (other.getTradeHandler().itemsTraded() > client.getActionAssistant().freeSlots()) {
			...
			if (!other.getTradeHandler().accepted()) {
				client.getActionSender().sendQuest("Waiting for other player...", 3431);
				other.getActionSender().sendQuest("Other player accepted.", 3431);
			} else {
				...
				other.getTradeHandler().setAccepted(false);
				client.getActionSender().sendQuest("Are you sure you want to accept this trade?", 3535);
				other.getActionSender().sendQuest("Are you sure you want to accept this trade?", 3535);
				sendOffer2();
				sendOtherOffer2();
	...
	public void sendOffer2() {
		...
		for (int i = 0; i < offer.length; i++) {
			...
			if (offer[i] > 0) {
				...
				trade.append(GameEngine.getItemManager().getItemDefinition(
						(offer[i] - 1)).getName());
				trade.append(" x ");
				trade.append(prefix);
				trade.append("\\n");
	...
	public void acceptStage2() {
		...
		if (currentTrade != null && currentTrade.isOpen()) {
			...
			if (!other.getTradeHandler().accepted()) {
				client.getActionSender().sendQuest("Waiting for other player...", 3535);
