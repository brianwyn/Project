...
public class Graphics extends ControllerManager {
	...
	public static void addNewRequest(Entity victim, int gfxId, int gfxHeight, int gfxDelay) {
	...
	public static void process() {
		...
		for (Graphics task : newList) {
			...
			if (task.getGfxDelay() == 0) {
				Animation.createGraphic(task.getVictim(), task.getGfxId(), 0, task.getGfxHeight() > 0);
				gfx.remove(task);
			}
		}
		newList.clear();
	...
	/**
	 * @param gfxHeight the gfxHeight to set
	 */
	...
	/**
	 * @param gfxDelay the gfxDelay to set
	 */
