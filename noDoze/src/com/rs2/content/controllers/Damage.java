package com.rs2.content.controllers;
...
public class Damage extends ControllerManager  {
	...
	public Damage(Entity attacker, Entity ent, CombatType type, int damage, int delay) {
	...
	public static void addNewHit(Entity attacker, Entity victim, CombatType type, int damage, int delay) {
	...
	public static void process() {
		...
		for (Damage newHit : newList) {
			...
			if (newHit.getHitDelay() == 0) {
				...
				if (newHit.getVictim().isBusy() && newHit.getVictim().getBusyTimer() > 1 ) {
					newHit.setHitDelay(1);
					continue;
				}
				if (Constants.FOOD_DELAY_HITS) {
					if (newHit.getVictim() instanceof Client) {
						if (((Client)newHit.getVictim()).foodDelay > 2 || ((Client)newHit.getVictim()).potionDelay > 1) {
							newHit.setHitDelay(1);
							continue;
						}
					}
				...
				if (newHit.getCombatType() != CombatType.THIEF && newHit.getCombatType() != CombatType.RECOIL && newHit.getCombatType() != CombatType.POISON) {
				
					if (newHit.getVictim() instanceof Client) {
					
						Client client = (Client) newHit.getVictim();
					
						if (Life.isAlive(newHit.getVictim()) && client.autoRetaliate && newHit.getVictim().getTarget() == null)
				...
				if (newHit.getVictim() instanceof Client) {
					...
					if (newHit.getAttacker() instanceof Client) {
						...
						if (newHit.getDamage() > 0) {
							if (newHit.getAttacker() instanceof Client) {
								if (Poison.canPoison(client2.playerEquipment[PlayerConstants.WEAPON]) && !Poison.isPoisoned(client)) {
									Poison.applyPoisonDesease(client, Poison.getPoisonSeverity(client2.playerEquipment[PlayerConstants.WEAPON]));
								}
							}
						}
					}
					client.getActionAssistant().appendHit(newHit.getCombatType(), newHit.getDamage());	
					
					if (client.isVengOn() && newHit.getCombatType() != CombatType.RECOIL) {
						if (newHit.getDamage() > 0)
							Lunar.vengenceEffect(client, newHit.getAttacker(), newHit.getDamage());
					}
					if (((Client)newHit.getVictim()).getEquipment().wearingRecoil() && newHit.getCombatType() != CombatType.RECOIL) { 
						Rings.applyRecoil(newHit.getVictim(), newHit.getAttacker(), Rings.pressDetails(newHit.getDamage(), 0), 0);
					}
