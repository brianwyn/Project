...
public class Animation extends ControllerManager {
	...
	public static void addNewRequest(Entity victim, int animationId, int animationDelay) {
		...
		for (Animation task : newList) {
			...
			if (task.getAnimationDelay() == 0) {
				...
				if (task.getVictim() instanceof Client)
					((Client)task.getVictim()).getActionSender().sendWindowsRemoval();
			}
		}
		newList.clear();
	}
	...
	public static void createGraphic(Entity ent, int gfx, int delay, boolean tallGfx) {
		if (ent == null || gfx < 1) 
			return;
		
		if (ent instanceof Client) {
			((Client)ent).getActionAssistant().createPlayerGfx(gfx, delay, tallGfx);
		} else if (ent instanceof NPC) {
			if (tallGfx) {
				((NPC)ent).gfx100(gfx);
			} else if (!tallGfx) {
	...
	public static void face(Entity attacker, Entity target) {
		...
		if (attacker instanceof Client) {
			if (target instanceof Client) {
				((Client)attacker).getActionAssistant().turnPlayerTo(32768 + ((Client)target).getUserID());
			} else if (target instanceof NPC) {
				((Client)attacker).getActionAssistant().turnPlayerTo(((NPC)target).getNpcId());
			}
	...
	public static void createAnimation(Entity attacker) {
		if (attacker == null) 
			return;
		if (attacker instanceof Client)
			createAnimation(attacker, Equipment.getAttackEmote((Client)attacker));
		else if (attacker instanceof NPC)
