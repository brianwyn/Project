...
public class Location extends ControllerManager {
	...
	public static void addNewRequest(Entity victim, int x, int y, int z, int delay) {
		...
		for (Location task : newList) {
			...
			if (task.getDelay() == 0) {
				...
				if (task.getVictim() instanceof Client) {
					...
					if (FightPits.inWaitingRoom(client))
						FightPits.removeFromWaitingRoom(client);
	...
	public static void to(Entity entity, int id, String teleType) {
		...
		if (entity instanceof Client) {
			...
			if (teleType == "magicbook") {
				if (client.playerLevel[PlayerConstants.MAGIC] < Magic.teleport(id).getLevel()) {
					client.getActionSender().sendMessage("You need a magic level of "+ Magic.teleport(id).getLevel() +" to cast this spell.");
					return;	
				}
				if (client.getWildernessLevel() >= 20) {
					client.getActionSender().sendMessage("You can't teleport above level "+ 20 +" wilderness.");
					return;	
				}
				...
				for (int i = 0; i < 4; i++) {
					if (runes[i] > 0) {
						if (!client.getActionAssistant().playerHasItem(runes[i], amount[i]) && !client.getActionAssistant().staffType(runes[i])) {
				...
				for (int i = 0; i < 4; i++) {
					if (runes[i] > 0) {
						if (!client.getActionAssistant().staffType(runes[i]))
							client.getActionAssistant().deleteItem(runes[i], amount[i]); 
					}	
				}
				Magic.playersProcessing.add(client);
				
				client.getActionAssistant().addSkillXP(Magic.teleport(id).getXp() * PlayerConstants.COMBAT_EXPERIENCE_MULTIPLIER, PlayerConstants.MAGIC);
				client.resetFaceDirection();
				client.resetWalkingQueue();
				entity.setBusyTimer(id > 7 ? 3 : 4);
				int s = client.getSpellBook();
				
				Graphics.addNewRequest(entity, s > 1 ? 392 : 308, s > 1  ? 0 : 100, s > 1  ? 1 : 3);
				Animation.addNewRequest(entity, s > 1  ? 1979 : 714, 0);
			
				if (s <= 1)
					Animation.addNewRequest(entity, 715, 5);
			
				Location.addNewRequest(entity, Magic.teleport(id).getAbsX() + Misc.random(2) - 1, Magic.teleport(id).getAbsY() + Misc.random(2) - 1, Magic.teleport(id).getHeight(), s > 1 ? 5 : 4);
				...
				Location.addNewRequest(entity, Magic.teleport(id).getAbsX(), Magic.teleport(id).getAbsY(), 0, 5);
