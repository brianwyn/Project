...
public class NPCFollow {
	...
	public static void runFollow(Entity npc, Entity following) {
		...
		if (actualDistance == 0) {
			...
			for (Tile tiles : testTiles) 
				if (FlagMap.locationOccupied(TileManager.currentLocation(tiles), npc))
					return;
			...
			for (Tile followTile : followingTiles) {
				for (Tile tiles : npcTiles) {
					...
					} else if (tiles.getTile()[1] == followTile.getTile()[1]) { //same y line
			...
			for (Tile northTile : north)
				if (FlagMap.locationOccupied(TileManager.currentLocation(northTile), npc)) 
					canGo[0] = true;
			for (Tile eastTile : east)
				if (FlagMap.locationOccupied(TileManager.currentLocation(eastTile), npc)) 
					canGo[1] = true;
			for (Tile southTile : south)
				if (FlagMap.locationOccupied(TileManager.currentLocation(southTile), npc)) 
					canGo[2] = true;	
			for (Tile westTile : west)
				if (FlagMap.locationOccupied(TileManager.currentLocation(westTile), npc)) 
					canGo[3] = true;
					
			for (Tile diagonalTile : diagonalTiles)
				if (FlagMap.locationOccupied(TileManager.currentLocation(diagonalTile), npc)) 
					invalidMove[0] = true;
			for (Tile sideway : xTiles) 
				if (FlagMap.locationOccupied(TileManager.currentLocation(sideway), npc)) 
					invalidMove[1] = true;
			for (Tile sideway2 : yTiles) 
				if (FlagMap.locationOccupied(TileManager.currentLocation(sideway2), npc)) 
					invalidMove[2] = true;
			
			for (int i = 0; i < canGo.length; i++) {
				if (canGo[i]) {
					if (location[1] < location2[1])
						if (i == 0) location[1] = locationCopy[1];
					if (location[1] > location2[1])
						if (i == 2) location[1] = locationCopy[1];
					if (location[0] > location2[0])
						if (i == 1) location[0] = locationCopy[0];
					if (location[0] < location2[0])
						if (i == 3) location[0] = locationCopy[0];
				}
			}
			for (int i = 0; i < invalidMove.length; i++) {
				if (invalidMove[i] == false) {
					foundPath = true;
					if (i == 0) {
						//still need to work out a 
						//movement system for this type, This is when it can go either way.
						break;
					} else if (i == 1) {
						location[0] = xLine[0];
						location[1] = xLine[1];
			...
			for (int i2 = 0; i2 < Constants.MAX_PLAYERS; i2++) {
				...
				for (Tile tiles : testTiles) {
					if (tiles.getTile()[0] == client.getAbsX() && tiles.getTile()[1] == client.getAbsY()) 
						return;
				}
			}
		} else if (actualDistance == 1 && validSidewardsWalker((NPC)npc) && !inMeleeDistance(npc, following)) {
			
			int randomSelection = Misc.random(1);
			int original = location[randomSelection];
			int opposite = randomSelection == 0 ? 1 : 0;
			
			location[randomSelection] = (location[randomSelection] > location2[randomSelection] ? location[randomSelection]-= 1 : location[randomSelection] < location2[randomSelection] ? location[randomSelection]+= 1 : location[randomSelection]);
		
			if (location[randomSelection] == original)
				location[opposite] = (location[opposite] > location2[opposite] ? location[opposite] -= 1 : location[opposite] < location2[opposite] ? location[opposite]+= 1 : location[opposite]);
	...
	public static boolean inMeleeDistance(Entity entity, Entity following) {
		...
		location[0] + 1 == location2[0] && location[1] == location2[1] ||
		location[1] - 1 == location2[1] && location[0] == location2[0] || 
		location[1] + 1 == location2[1] && location[0] == location2[0];
	}
