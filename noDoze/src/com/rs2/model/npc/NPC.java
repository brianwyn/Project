...
import com.rs2.world.XMLManager.NPCSizes;
...
public class NPC extends Entity {
	...
	public void hit(Entity attacker, Entity victim, int damage) {
		...
		if (attacker instanceof Client) {
			if (victim instanceof NPC) {
				if (((NPC) victim).attackers.get(((Client) attacker)) != null) {
					hitTotal = ((NPC) victim).attackers.get((Client) attacker);
				}
				if (damage > ((NPC) victim).hp) {
					damage = ((NPC) victim).hp;
				}
				if (!hitUpdateRequired) {
					((NPC) victim).attackers.put(((Client) attacker).getUsername(), hitTotal + damage);
					...
					Animation.createAnimation(victim, NPCManager.emotions[npcId][1]);
					victim.walkingHome = false;
					this.following = (Client) attacker;
					
				} else if (!hitUpdateRequired2) {
					((NPC) victim).attackers.put(((Client) attacker).getUsername(), hitTotal
							+ damage);
					...
					Damage.addNewHit(attacker, victim,CombatType.MELEE, damage, 0);
				}
				this.updateRequired = true;
			}
		}
	 ...
	 * opitmized by killamess to
	 * produce a single random drop
	 */
	public void processDrops() {
		...
		if (highestHitter == null) {
			...
			List<NPCDrop> drop = definition.getDrops();
			...
			boolean non100[] = new boolean[drop.size()];
			int dropCount = 0;
			
			for (NPCDrop drops : drop) {
				if (drops.getChance() < 100) 
			...
			for (int i = 0; i < non100.length; i++) {
				...
				if (!non100[i]) {
					FloorItem item = new FloorItem(drop.get(i).getId(), drop.get(i).getAmount(), highestHitter, getAbsX(), getAbsY(), getHeightLevel());
					toDrop.add(item);
				}
			}
			if (!random.isEmpty()) {
				int randomSelection = Misc.random(random.size());
				for (int i : random) {
					if (i == randomSelection) {
						if (Misc.random(100) < drop.get(i).getChance()) {
							FloorItem newRandomDrop = new FloorItem(drop.get(i).getId(), drop.get(i).getAmount(), highestHitter, getAbsX(), getAbsY(), getHeightLevel());
		...
		if (highestHitter != null) {
			...
			case 173:
			case 442:
				GameEngine.getGlobalActions().sendMessage(""+this.getDefinition().getName()+" has been slain by: "+highestHitter+"");
			}
		}
	...
	public void process() {
		...
		/*
		if (!isDead && TileManager.calculateDistance(TileManager.currentLocation(this), NPCFollow.getHomeLocation(this)) > 10) {
			Location.addNewRequest(this, NPCFollow.getHomeLocation(this)[0], NPCFollow.getHomeLocation(this)[1], NPCFollow.getHomeLocation(this)[2], 0);
			...
			if (getDefinition().getType() == Constants.PET_TYPE) {
				if (getOwner() != null && !((Client)getOwner()).disconnected) {
					...
					if (TileManager.calculateDistance(this, getOwner()) > 16) {
						Location.addNewRequest(this,	getOwner().getAbsX(), getOwner().getAbsY(), getOwner().getHeightLevel(), 0);
						return;
					}
			...
			if (pathCreated && stepsAllocated > 0 && ent.getTarget() == null) {
				...
				if (IsInRange(this, nextStep[0], nextStep[1]) == false) {
					...
					for (Tile tiles : testTiles) 
						if (FlagMap.locationOccupied(TileManager.currentLocation(tiles), this))
							canMove = false;
	 ...
	 */
	public static void newNPC(int id, int x, int y, int z, Entity theOwner) {
