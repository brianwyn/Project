...
public class Equipment {
	...
	private void sendSpecialBar() {
		int id = client.playerEquipment[PlayerConstants.WEAPON];
		switch (id) {
	...
	public void sendWeapon() {
		int weapon = client.playerEquipment[PlayerConstants.WEAPON];
		String s = GameEngine.getItemManager().getItemDefinition(weapon).getName();
		if (s.equals("Unarmed")) {
		...
		} else if (s.startsWith("Staff") || s.endsWith("staff") || s.endsWith("wand")) {
		...
		} else if (s.endsWith("dart") || s.endsWith("knife") || s.endsWith("javelin") || s.contains("thrownaxe")) {
	...
	public void wearItem(int wearID, int slot) {
		...
		if (strenghtReq > client
			...
			client
					.getActionSender()
					.sendMessage(
							(new StringBuilder()).append("You need ").append(
									strenghtReq).append(
									" Strength to equip this item.").toString());
			return;
		}
		if (defenceReq > client
			...
			client.getActionSender().sendMessage(
					(new StringBuilder()).append("You need ")
							.append(defenceReq).append(
									" Defence to equip this item.").toString());
			return;
		}
		if (magicReq > client
				.getLevelForXP(client.playerXP[PlayerConstants.MAGIC])) {
		...
		if (slot > -1 && slot < 28 && client.playerItems[slot] == wearID + 1) {
			...
			if (j == PlayerConstants.WEAPON) {
				...
				sendWeapon();
				client.combatMode = CombatMode.setCombatMode(client, client.combatMode);
			}
			client.getBonuses().calculateBonus();
			client.updateRequired = true;
			client.appearanceUpdateRequired = true;
	...
	public static int getStandEmote(Client client) {
		int i = client.playerEquipment[PlayerConstants.WEAPON];
		String s = GameEngine.getItemManager().getItemDefinition(i).getName();
		s = s.replaceAll("_", " ");
		s = s.trim();
		if (i == -1)
		...
		if (s.contains("bow"))
			return 808;	
		if (s.contains("2h") || s.endsWith("godsword") || s.equals("Saradomin sword"))
			return 7047;
		if (s.contains("whip"))
			return 1832;
		if (s.endsWith("spear"))
	...
	public static int getAttackEmote(Client client) {
		...
		if (s.contains("2h") || s.contains("godsword") || s.equals("Saradomin sword")) {
			if (client.combatMode == 0) {
				return 7048;
		...
		if (s.contains("sword") && !s.contains("2h") || s.contains("mace") || s.contains("longsword") && !s.contains("2h") || s.contains("scimitar")) {
		...
		if (s.contains("axe") && !s.contains("greataxe") && !s.contains("thrownaxe") || s.contains("battleaxe"))
			return 395;
		if (s.contains("halberd") || s.contains("spear") && !s.contains("Guthans"))
			return 440;
	...
	public static int getWeaponSpeed(Client client) {
		...
		if (s.contains("2h") || s.contains("god"))
			return 7;
		if (s.contains("sword") && !s.contains("longsword") && !s.contains("god") && basicId(i))
			return 5;
		...
		if (s.contains("mace") && basicId(i))
			return 6;
		if (s.contains("knife") || s.contains("dart") && basicId(i) || s.contains("thrownaxe") && basicId(i)) {
		...
		if (s.contains("shortbow") || s.contains("Shortbow") || s.equals("Seercull") || s.equals("Karil's crossbow") || s.equals("Toktz-xil-ul")) {
			if (client.combatMode == 4) {
				return 5;
			} else 	if (client.combatMode == 5) {
				return 4;
		...
		if (s.contains("Crossbow") || s.equals("Crystal bow full") || s.equals("New crystal bow") || s.contains("javelin") && basicId(i)) {
			if (client.combatMode == 4) {
				return 7;
			} else 	if (client.combatMode == 5) {
				return 6;
		...
		if (s.contains("longbow") || s.contains("Longbow") || s.contains("c'bow")) {
			if (client.combatMode == 4) {
				return 7;
			} else 	if (client.combatMode == 5) {
				return 6;
		...
		if (s.contains("axe") && !s.contains("greataxe") && !s.contains("battleaxe") && !s.contains("pickaxe") && basicId(i))
			return 6;
	...
	public static int getDefendAnimation(Client client) {
		int weapon = client.playerEquipment[PlayerConstants.WEAPON];
		int shield = client.playerEquipment[PlayerConstants.SHIELD];
		int i = client.playerEquipment[PlayerConstants.WEAPON];
		String s = GameEngine.getItemManager().getItemDefinition(i).getName();
		s = s.replaceAll("_", " ");
		s = s.trim();
