...
import com.rs2.model.combat.content.BarrowsEffects;
...
public class ActionAssistant {
    ...
    public void showNewLevel(int skillId) {
    	client.getActionSender().sendMessage("Congratulations, you just advanced an " + PlayerConstants.SKILL_NAMES[skillId].toLowerCase() + " level!");
	...
	public void addSkillXP(double amount, int skillId) {
		...
		if (oldLevel < client.getLevelForXP(client.playerXP[skillId])) {
			client.playerLevel[skillId] = client.getLevelForXP(client.playerXP[skillId]);
	...
	public void setFollow(boolean objective, int followID) {
		if (followID > 0 && objective) {
			if (PlayerManager.getSingleton().getPlayers()[followID] != null) {
				client.followPlayer = ((Client) PlayerManager.getSingleton().getPlayers()[followID]);
	...
	public void dropItem(int id, int slot) {
		if (client.playerItems[slot] == (id + 1)) {
			FloorItem i = new FloorItem(client.playerItems[slot] - 1,
					client.playerItemsN[slot], client.getUsername(), client.getAbsX(), client
	...
	public void makeFloorItem(int id, int amount) {
		FloorItem i = new FloorItem(id, amount, client.getUsername(), client.getAbsX(), client.getAbsY(), client.getHeightLevel());
		GameEngine.getItemManager().newDrop(i, client);
	}
