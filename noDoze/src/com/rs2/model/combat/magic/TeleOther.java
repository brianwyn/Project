...
public class TeleOther {
	...
	public static void castSpell(Client castingPlayer, Client effectedPlayer, int spell) {
		
		if (TileManager.calculateDistance(castingPlayer, effectedPlayer) > 16) {
			castingPlayer.getActionSender().sendMessage("This "+ effectedPlayer.getUsername() +" is too far away.");
			return;
		}
		...
		for (int[] s : SPELLS) {
			if (s[0] == spell) {
				...
				if (!castingPlayer.getActionAssistant().playerHasItem(s[2], s[3]) && !castingPlayer.getActionAssistant().staffType(s[2]) || 
					!castingPlayer.getActionAssistant().playerHasItem(s[4], s[5]) && !castingPlayer.getActionAssistant().staffType(s[4]) ||
					!castingPlayer.getActionAssistant().playerHasItem(s[6], s[7]) && !castingPlayer.getActionAssistant().staffType(s[6])) {
				...
				effectedPlayer.getActionSender().sendQuest(castingPlayer.getUsername(), 12558);
				effectedPlayer.getActionSender().sendQuest(getTeleName(s[0]), 12560);
			}
		}	
	}
	...
	public static void acceptTeleport(Client toMove, Client toCast, int spell) {
		...
		if (TileManager.calculateDistance(toMove, toCast) > 16) {
			toMove.getActionSender().sendMessage("You are too far away from "+ toCast.getUsername() +".");
			return;	
		}
