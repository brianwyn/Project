...
public class MagicHandler extends Magic {
	...
	public static void createProjectile(Entity attacker, Entity victim, int spellId) {
		...
		if (attacker instanceof Client) {
			((Client)attacker).getActionAssistant().addSkillXP(((spell(spellId).getMagicLevel()) * PlayerConstants.COMBAT_EXPERIENCE_MULTIPLIER) / 10, PlayerConstants.MAGIC);
			if (victim instanceof Client && projectileId > 0)
				GameEngine.getGlobalActions().sendProjectile(((Client)attacker), attacker.getAbsY(), attacker.getAbsX(), offsetY, offsetX, projectileId, startHeight, endHeight, projectileSpeed, 17, - ((Client)victim).getUserID() -1);
			
			else if (victim instanceof NPC && projectileId > 0) 
				GameEngine.getGlobalActions().sendProjectile(((Client)attacker), attacker.getAbsY(), attacker.getAbsX(), offsetY, offsetX, projectileId, startHeight, endHeight, projectileSpeed, 17, ((NPC)victim).getNpcId() + 1);
		
		} else if (attacker instanceof NPC) {
			
			if (victim instanceof Client && projectileId > 0)
				GameEngine.getGlobalActions().sendProjectile(((Client)victim), attacker.getAbsY(), attacker.getAbsX(), offsetY, offsetX, projectileId, startHeight, endHeight, projectileSpeed, 17, - ((Client)victim).getUserID() -1);
		}
		...
		totalDamage = Misc.random(damage + extraDamage);
		
		int hitDelay = (spell(spellId).getEndHitDelay() - (spell(spellId).getProjectileDelay() > 0 ? 1 : 0));
		...
		if (hasHit) 
			Damage.addNewHit(attacker, victim, attacker.getCombatType(), totalDamage, hitDelay + (hasProjectile ? 0 : distance / 4));
		...
		if (attacker instanceof Client) {
			...
			if (victim instanceof Client) {
				if (((Client)victim).getEquipment().wearingRecoil()) 
					Rings.applyRecoil(victim, attacker, Rings.pressDetails(totalDamage, 0),  hitDelay + (hasProjectile ? 0 : distance / 4));
				if (((Client)attacker).getPrayerHandler().clicked[17]) 
					((Client)victim).getPrayerHandler().updatePrayer(totalDamage / 4);
			}
			if (client.usingMagicDefence) {
				client.getActionAssistant().addSkillXP((totalDamage * PlayerConstants.COMBAT_EXPERIENCE_MULTIPLIER) / 2, PlayerConstants.MAGIC);
				client.getActionAssistant().addSkillXP((totalDamage * PlayerConstants.COMBAT_EXPERIENCE_MULTIPLIER) / 2, PlayerConstants.DEFENCE);
				client.getActionAssistant().addSkillXP((totalDamage * PlayerConstants.COMBAT_EXPERIENCE_MULTIPLIER) / 3, PlayerConstants.HITPOINTS);
			} else {
				client.getActionAssistant().addSkillXP(totalDamage * PlayerConstants.COMBAT_EXPERIENCE_MULTIPLIER, PlayerConstants.MAGIC);
				client.getActionAssistant().addSkillXP((totalDamage * PlayerConstants.COMBAT_EXPERIENCE_MULTIPLIER) / 3, PlayerConstants.HITPOINTS);
			}
		}
		if (MagicType.getMagicType(spellId) == MagicType.type.FREEZE || MagicType.getMagicType(spellId) == MagicType.type.BIND)
	...
	public static void endGfx(Entity victim, Entity attacker, int spellId) {
		...
		if (gfx > 0) {
			...
			if (hasHit && !fail) 
				Graphics.addNewRequest(victim, gfx, gfxHeight, gfxDelay + (hasProjectile ? 0 : distance / 4));
			else
				Graphics.addNewRequest(victim, 85, 100, gfxDelay + (hasProjectile ? 0 : distance / 4));
		}
