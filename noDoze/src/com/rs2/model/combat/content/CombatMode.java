...
public class CombatMode {
	...
	public static String[] combatModes = { "Attack", "Defence", "Strength", "Controlled", "Accurate", "Rapid", "LongRanged" };
	...
	public static int findCombatMode(Client client, int weapon) {
		for (int i = 0; i < 6; i++) {
			if (i < 4 && hasCombatMode(weapon, i) && client.lastRangeMode == i) {
				return i;
			} else if (i < 4 && hasCombatMode(weapon, i) && client.lastMeleeMode == -1) {
				client.lastMeleeMode = i;
				return i;
			}
			if (i > 3 && hasCombatMode(weapon, i) && client.lastRangeMode == i) {
				return i;
			} else if (i > 3 && hasCombatMode(weapon, i) && client.lastRangeMode == -1) {
	...
	public static void setConfigId(Client client, int mode) {
		...
		int boxCount = getBoxCount(client.playerEquipment[PlayerConstants.WEAPON], mode);
