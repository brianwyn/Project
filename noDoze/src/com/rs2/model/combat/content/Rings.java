...
public class Rings {
	...
	public static void applyRecoil(Entity attacker, Entity target, int[] damage, int delay) {
			...
			if (((Client)attacker).getRecoilCount() > 1) {
				for (int amount : damage) {
					if (((Client)attacker).getRecoilCount() - amount >= 1)
						((Client)attacker).setRecoilCount(((Client)attacker).getRecoilCount() - amount);
					else
		...
		if (inUse[0])
			Damage.addNewHit(attacker, target, CombatType.RECOIL, finalDamage[0], delay);
		if (inUse[1]) 
			Damage.addNewHit(attacker, target, CombatType.RECOIL, finalDamage[1], delay);
	}
	...
	public static void applyRingOfLife(Entity entity, int damage) {
		...
		if (client != null) {
			...
			if (client.playerEquipment[PlayerConstants.RING] != 2570)
				return;
			...
			if (client.hitpoints <= fifteenPercent && client.hitpoints > 0) {
				Location.to(client, 7, "magicbook");
				breakRing(client);
			}
		}		
	}
	...
	public static void breakRing(Entity ent) {
		if (ent instanceof Client) {
			...
			((Client)ent).getActionSender().sendMessage("Your ring crumbles to dust.");
		}
	}
