...
public class Specials {
		...
		for (int[] slot : SPECIAL) {
			if (client.playerEquipment[PlayerConstants.WEAPON] == slot[0]) {
				if (slot[0] == 4151 && target != null) {
					Animation.createGraphic(target, slot[2], 0, slot[0] != 7158);
				} else {
					Animation.createGraphic(client, slot[2], 0, slot[0] != 7158);
				}
	...
	public static void activateSpecial(Client client) {
		...
		case 1377: //dba
			...
			Animation.createGraphic(client, getSpecialGraphics(client), 0, false);
			turnOff(client);
			break;
		case 4153: //g mual
			if (client.getTarget() != null && TileManager.calculateDistance(client.getTarget(), client) < 2 && Life.isAlive(client.getTarget())) {
				...
				turnOff(client);
			} else if (client.getTarget() == null && client.usingSpecial() || client.getTarget() != null && TileManager.calculateDistance(client.getTarget(), client) > 1 && client.usingSpecial()) {
				client.getActionSender().sendMessage("Warning: Since the maul's special is an instant attack, It will be wasted when used on a");
	...
	public static void updateSpecialBar(Client client) {
		client.getActionSender().sendConfig2(300, client.getSpecialAmount() * 10);
	}
