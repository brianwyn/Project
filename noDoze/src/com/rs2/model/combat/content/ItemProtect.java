...
public class ItemProtect {
	...
	public static final int[] protectedList = {
		6570, 8844, 8845, 8846, 8847, 8848, 8849, 8850, 
		10551, 7455, 7456, 7457, 7458, 7459, 7460, 7461, 7462,
		10498, 10499, 11665, 8839, 8840, 8842, 11664, 9747, 9748, 
		9753, 9754, 9750, 9751, 9768, 9769, 9756, 9757, 9759, 9760, 
		9762, 9763, 9801, 9802, 9807, 9808, 9783, 9784, 9798, 9799, 
		9804, 9805, 9780, 9781, 9795, 9796, 9792, 9793, 9774, 9775, 
		9771, 9772, 9777, 9778, 9786, 9787, 9810, 9811, 9765, 9766, 
		9789, 9790, 9948, 9949, 9813, 9813, 
	};
	...
	public static void add(Client client, int loopCount, int item, int value, int amount) {
	...
	public static int[] dropItems(Client client, boolean dropItems) {
		...
		for (int i = 0; i < 28; i++)
			add(client, i, client.playerItems[i] - 1, (int) GameEngine.getItemManager().getItemDefinition(client.playerItems[i] - 1).getShopValue(), client.playerItemsN[i]);

		for (int i = 0; i < 14; i++) 
			add(client, i + 28, client.playerEquipment[i], (int) GameEngine.getItemManager().getItemDefinition(client.playerEquipment[i]).getShopValue(), client.playerEquipmentN[i]);
		...
		if (dropItems) {
			...
			for (int i = 0; i < dropsToAdd.length; i++) {
				...
				for (int protect : protectedList) {
					if (dropsToAdd[i][0] == protect) {
						GameEngine.getItemManager().newDrop(new FloorItem(dropsToAdd[i][0], dropsToAdd[i][1], client.getUsername(), client.getAbsX(), client.getAbsY(), client.getHeightLevel()), client);
						dropsToAdd[i][0] = 0;
						dropsToAdd[i][1] = 0;
					} 
				}
			...
			for (int i = 0; i < dropsToAdd.length; i++)  {
				if (dropsToAdd[i][0] > 0) {
					GameEngine.getItemManager().newDrop(new FloorItem(dropsToAdd[i][0], dropsToAdd[i][1], client.whoKilledYa == null ? client.getUsername() : client.whoKilledYa.getUsername(), client.getAbsX(), client.getAbsY(), client.getHeightLevel()), client.whoKilledYa == null ? client : client.whoKilledYa);
				}
			}
			GameEngine.getItemManager().newDrop(new FloorItem(526, 1, client.whoKilledYa == null ? client.getUsername() : client.whoKilledYa.getUsername(), client.getAbsX(), client.getAbsY(), client.getHeightLevel()), client.whoKilledYa == null ? client : client.whoKilledYa);		
